...
// AUTH HELPERS
// ─────────────────────────────────────────────────────────────────────────────
const getUsers = () => { try { return JSON.parse(localStorage.getItem("todo-users") || "[]"); } catch { return []; } };
const saveUsers = (u) => localStorage.setItem("todo-users", JSON.stringify(u));
const getSession = () => { try { return JSON.parse(localStorage.getItem("todo-session") || "null"); } catch { return null; } };
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, fontFamily: "'Segoe UI',system-ui,sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
        {[{ top: "-20%", left: "-10%", s: 600, c: dark ? "rgba(99,102,241,0.14)" : "rgba(99,102,241,0.18)" },
          { bottom: "-15%", right: "-10%", s: 500, c: dark ? "rgba(236,72,153,0.09)" : "rgba(236,72,153,0.13)" }
        ].map((o, i) => (
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ background: T.card, backdropFilter: "blur(20px)", borderRadius: 28, border: `1.5px solid ${T.bord}`, padding: "32px 28px", boxShadow: dark ? "0 24px 80px rgba(0,0,0,0.5)" : "0 24px 80px rgba(99,102,241,0.13)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {mode === "signup" && (
              <div>
                ...
                <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Satyam Yadav" onKeyDown={e => e.key === "Enter" && handle()} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${T.bord}`, background: T.inp, color: T.text, fontSize: 15, fontWeight: 500, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
              </div>
            )}
            <div>
              ...
              <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" type="email" onKeyDown={e => e.key === "Enter" && handle()} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${T.bord}`, background: T.inp, color: T.text, fontSize: 15, fontWeight: 500, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
            </div>
          ...
          <p style={{ textAlign: "center", fontSize: 12, color: T.muted, marginTop: 20, lineHeight: 1.8 }}>
            ...
            <span onClick={() => { setMode(mode === "login" ? "signup" : "login"); setError(""); }} style={{ color: T.sub, fontWeight: 700, cursor: "pointer" }}>{mode === "login" ? "Sign up free" : "Log in"}</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10002, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: dark ? "#13131f" : "#fff", borderRadius: 28, padding: 28, maxWidth: 380, width: "100%", border: `1.5px solid ${T.bord}`, boxShadow: "0 24px 80px rgba(0,0,0,0.5)", animation: "popIn 0.35s cubic-bezier(.34,1.56,.64,1)", fontFamily: "'Segoe UI',system-ui,sans-serif" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
          {[{ n: total, l: "Total", c: "#6366f1", bg: dark ? "rgba(99,102,241,0.18)" : "#eef2ff" },
            { n: done, l: "Done", c: "#10b981", bg: dark ? "rgba(16,185,129,0.18)" : "#ecfdf5" },
            { n: total - done, l: "Left", c: "#f59e0b", bg: dark ? "rgba(245,158,11,0.18)" : "#fefce8" }
          ].map(({ n, l, c, bg }) => (
...
function getBestVoice(type = "male") {
  ...
  if (type === "female") {
    return voices.find(v => v.lang.startsWith("en") && (v.name.toLowerCase().includes("female") || v.name.includes("Zira") || v.name.includes("Samantha") || v.name.includes("Karen"))) || voices.find(v => v.lang.startsWith("en")) || voices[0];
  }
  return voices.find(v => v.lang.startsWith("en") && (v.name.toLowerCase().includes("male") || v.name.includes("David") || v.name.includes("Daniel") || v.name.includes("Alex"))) || voices.find(v => v.lang.startsWith("en")) || voices[0];
}
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: dark ? "#13131f" : "#fff", borderRadius: 32, padding: "36px 28px", maxWidth: 400, width: "100%", border: "2px solid #7c3aed", boxShadow: "0 0 0 8px rgba(124,58,237,0.12),0 32px 80px rgba(0,0,0,0.6)", textAlign: "center", animation: "popIn 0.45s cubic-bezier(.34,1.56,.64,1)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <span style={{ fontSize: 13, fontWeight: 700, padding: "5px 14px", borderRadius: 99, color: "#fff", background: cm ? cm.gradient : "linear-gradient(135deg,#6366f1,#818cf8)" }}>{cm ? cm.icon : "✨"} {todo.category}</span>
          {todo.alarm && <span style={{ fontSize: 13, fontWeight: 700, padding: "5px 14px", borderRadius: 99, color: "#7c3aed", background: dark ? "rgba(124,58,237,0.15)" : "#ede9fe", border: "1px solid #ddd6fe" }}>🕐 {todo.alarm}</span>}
          <span style={{ fontSize: 13, fontWeight: 700, padding: "5px 14px", borderRadius: 99, color: todo.voiceType === "female" ? "#ec4899" : "#3b82f6", background: todo.voiceType === "female" ? (dark ? "rgba(236,72,153,0.15)" : "#fdf2f8") : (dark ? "rgba(59,130,246,0.15)" : "#eff6ff"), border: `1px solid ${todo.voiceType === "female" ? "#fbcfe8" : "#bfdbfe"}` }}>{todo.voiceType === "female" ? "👩 Female" : "👨 Male"}</span>
        </div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10001, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: T.bg, borderRadius: 28, padding: 28, maxWidth: 460, width: "100%", border: `1.5px solid ${T.bord}`, boxShadow: "0 24px 80px rgba(0,0,0,0.5)", animation: "popIn 0.35s cubic-bezier(.34,1.56,.64,1)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", color: T.muted, lineHeight: 1 }}>✕</button>
        </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {[{ val: "male", icon: "👨", label: "Male Voice", color: "#3b82f6", bg: dark ? "rgba(59,130,246,0.15)" : "#eff6ff", bord: "#bfdbfe" },
              ...
              <button key={v.val} onClick={() => setVoiceType(v.val)} style={{ flex: 1, padding: "12px 8px", borderRadius: 14, cursor: "pointer", border: `2px solid ${voiceType === v.val ? v.color : T.bord}`, background: voiceType === v.val ? v.bg : (dark ? "transparent" : "#fafafa"), transition: "all 0.2s" }}>
                ...
                {voiceType === v.val && (<button onClick={e => { e.stopPropagation(); speakAlarm(text || "Test", v.val); }} style={{ marginTop: 6, padding: "4px 10px", borderRadius: 8, border: "none", background: v.color, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>🔊 Test</button>)}
              </button>
            ))}
          </div>
        </div>
      ...
      {subjects.length > 0 && (
        <div style={{ background: T.card, backdropFilter: "blur(20px)", borderRadius: 28, border: `1px solid ${T.bord}`, padding: 22, marginBottom: 16, boxShadow: T.shadow }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#10b981", background: dark ? "rgba(16,185,129,0.15)" : "#ecfdf5", padding: "5px 14px", borderRadius: 99, border: "1px solid #a7f3d0" }}>{doneTopics} done</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#6366f1", background: dark ? "rgba(99,102,241,0.15)" : "#eef2ff", padding: "5px 14px", borderRadius: 99, border: "1px solid #c7d2fe" }}>{totalTopics} total</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#f59e0b", background: dark ? "rgba(245,158,11,0.15)" : "#fefce8", padding: "5px 14px", borderRadius: 99, border: "1px solid #fde68a" }}>{totalTopics - doneTopics} left</span>
          </div>
        </div>
      )}
...
export default function TodoApp() {
  ...
  useEffect(() => {
    if (!currentUser) { setSubjects([]); return; }
    try {
      const saved = JSON.parse(localStorage.getItem(`study-v1-${currentUser.id}`));
      setSubjects(saved || []);
    } catch { setSubjects([]); }
  ...
  useEffect(() => {
    if (!currentUser) { setTodos(DEFAULTS); return; }
    try {
      const saved = JSON.parse(localStorage.getItem(`todo-v6-${currentUser.id}`));
      setTodos(saved || DEFAULTS);
    } catch { setTodos(DEFAULTS); }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUser?.id]);
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 640, margin: "0 auto", padding: "28px 16px 100px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => setShowProfile(true)} title={currentUser.name} style={{ width: 46, height: 46, borderRadius: 15, border: "2px solid #7c3aed", background: "linear-gradient(135deg,#6366f1,#ec4899)", cursor: "pointer", fontSize: 14, fontWeight: 900, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(99,102,241,0.4)" }}>{initials}</button>
          </div>
        </div>
        ...
        {/* TAB SWITCHER */}
        <div style={{ display: "flex", gap: 4, background: dark ? "rgba(255,255,255,0.05)" : "#ede9fe", borderRadius: 16, padding: 4, marginBottom: 16 }}>
          {[{ id: "tasks", label: "✅ Tasks" }, { id: "study", label: "📚 Study Tracker" }].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ flex: 1, padding: "11px", borderRadius: 12, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, transition: "all 0.2s", background: activeTab === tab.id ? "linear-gradient(135deg,#6366f1,#ec4899)" : "transparent", color: activeTab === tab.id ? "#fff" : T.muted, boxShadow: activeTab === tab.id ? "0 4px 14px rgba(99,102,241,0.35)" : "none" }}>{tab.label}</button>
          ))}
        </div>
        ...
        {activeTab === "study" && (
          <StudyTracker
            ...
            dark={dark}
            showAddSubject={showAddSubject} setShowAddSubject={setShowAddSubject}
            subjectName={subjectName} setSubjectName={setSubjectName}
            subjectTotal={subjectTotal} setSubjectTotal={setSubjectTotal}
          />
        )}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
          {CATEGORIES.map(c2 => {
            ...
            return (<button key={c2} onClick={() => setFilter(c2)} style={{ padding: "9px 18px", borderRadius: 14, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, transition: "all 0.2s", background: a ? (m ? m.gradient : "linear-gradient(135deg,#6366f1,#818cf8)") : T.glass, color: a ? "#fff" : T.muted, backdropFilter: "blur(10px)", boxShadow: a ? `0 4px 16px ${m ? m.glow : "rgba(99,102,241,0.3)"}` : "none", transform: a ? "translateY(-2px)" : "none" }}>{m ? `${m.icon} ${c2}` : `✨ ${c2}`}</button>);
          })}
          <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
            {PRIORITIES.map(p2 => {
              const a = priFilter === p2; const m = PRI_META[p2];
              return (<button key={p2} onClick={() => setPriFilter(priFilter === p2 ? "All" : p2)} style={{ padding: "7px 12px", borderRadius: 12, cursor: "pointer", fontSize: 12, fontWeight: 700, border: `1.5px solid ${a ? m.color : T.bord}`, background: a ? m.bg : T.glass, color: a ? m.color : T.muted, backdropFilter: "blur(10px)" }}>{m.label}</button>);
            })}
          </div>
        </div>
        ...
        <div style={{ background: T.card, backdropFilter: "blur(20px)", marginBottom: 14, border: `1.5px solid ${showAdd ? "#7c3aed" : T.bord}`, borderRadius: 24, overflow: "hidden", boxShadow: showAdd ? "0 12px 40px rgba(124,58,237,0.2)" : T.shadow, transition: "all 0.3s" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px" }}>
            <div onClick={() => setShowAdd(s => !s)} style={{ width: 36, height: 36, borderRadius: 12, flexShrink: 0, cursor: "pointer", background: "linear-gradient(135deg,#6366f1,#ec4899)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, color: "#fff", fontWeight: 700, lineHeight: 1, boxShadow: "0 4px 14px rgba(99,102,241,0.4)" }}>{showAdd ? "−" : "+"}</div>
            ...
            <div style={{ padding: "4px 18px 18px", borderTop: `1px solid ${T.bord}` }}>
              ...
              <div style={{ background: dark ? "rgba(124,58,237,0.1)" : "#faf5ff", border: `1.5px solid ${dark ? "rgba(124,58,237,0.25)" : "#ddd6fe"}`, borderRadius: 18, padding: 16 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[{ val: "male", icon: "👨", label: "Male", color: "#3b82f6", bg: dark ? "rgba(59,130,246,0.15)" : "#eff6ff", bord: "#bfdbfe" }, { val: "female", icon: "👩", label: "Female", color: "#ec4899", bg: dark ? "rgba(236,72,153,0.15)" : "#fdf2f8", bord: "#fbcfe8" }].map(v => (
              ...
              <div key={todo.id} draggable onDragStart={() => onDragStart(todo.id)} onDragOver={e => onDragOver(e, todo.id)} onDrop={() => onDrop(todo.id)} onDragEnd={() => { setDragId(null); setDragOver(null); }} style={{ background: todo.done ? (dark ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.6)") : T.card, backdropFilter: "blur(20px)", borderRadius: 20, border: `1.5px solid ${isDragTarget ? "#6366f1" : hasAlarm ? "rgba(124,58,237,0.4)" : T.bord}`, padding: "15px 16px", display: "flex", alignItems: "flex-start", gap: 12, opacity: dragId === todo.id ? 0.3 : 1, transform: isDragTarget ? "scale(1.02)" : "scale(1)", transition: "all 0.2s ease", boxShadow: hasAlarm ? (dark ? "0 6px 28px rgba(124,58,237,0.22)" : "0 6px 28px rgba(124,58,237,0.14)") : todo.done ? "none" : T.shadow, cursor: "grab", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 9, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {todo.alarm && !todo.done && <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 99, fontWeight: 700, background: dark ? "rgba(124,58,237,0.18)" : "#faf5ff", color: "#7c3aed", border: "1.5px solid #ddd6fe", display: "flex", alignItems: "center", gap: 5 }}>
                      {isFemale ? "👩" : "👨"} 🔔 {todo.alarm}
