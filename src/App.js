...
function getBestVoice(type = "male") {
  ...
  if (type === "female") {
    return voices.find(v => v.lang.startsWith("en") &&
      (v.name.toLowerCase().includes("female") || v.name.includes("Zira") ||
...
function playBeep() {
  try {
    ...
    [[660,0.0,0.18],[660,0.25,0.18],[880,0.50,0.30]].forEach(([f,t,d]) => {
...
function speakAlarm(taskText, voiceType="male") {
  ...
  setTimeout(() => {
    ...
    const say = () => {
      ...
      utt.rate    = voiceType === "female" ? 0.85 : 0.80;
      utt.pitch   = voiceType === "female" ? 1.10 : 0.70;
      utt.volume  = 1.0;
      utt.onend   = () => {
        setTimeout(() => {
          const u2   = new SpeechSynthesisUtterance(taskText);
          u2.voice   = getBestVoice(voiceType);
          u2.rate    = voiceType === "female" ? 0.85 : 0.80;
          u2.pitch   = voiceType === "female" ? 1.10 : 0.70;
          u2.volume  = 1.0;
...
function Confetti({ active, onDone }) {
  const ref=useRef(null); const cb=useRef(onDone); cb.current=onDone;
  useEffect(()=>{
    if(!active)return;
    ...
    const pp=Array.from({length:160},()=>({
      x:Math.random()*cv.width,y:-20,w:5+Math.random()*10,h:3+Math.random()*7,
      color:cols[Math.floor(Math.random()*cols.length)],
      vx:(Math.random()-.5)*5,vy:2+Math.random()*6,angle:Math.random()*360,va:(Math.random()-.5)*10,
    }));
    let raf;
    const draw=()=>{
      ctx.clearRect(0,0,cv.width,cv.height);let done=true;
      pp.forEach(p=>{p.x+=p.vx;p.y+=p.vy;p.angle+=p.va;
    ...
    draw();const t=setTimeout(()=>cb.current(),4500);
...
function AlarmPopup({ alarms, onDismiss, onSnooze, dark }) {
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,0.8)",
      backdropFilter:"blur(10px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:dark?"#13131f":"#fff",borderRadius:32,padding:"36px 28px",
        maxWidth:400,width:"100%",border:"2px solid #7c3aed",
        boxShadow:"0 0 0 8px rgba(124,58,237,0.12),0 32px 80px rgba(0,0,0,0.6)",
        textAlign:"center",animation:"popIn 0.45s cubic-bezier(.34,1.56,.64,1)"}}>
        <div style={{width:90,height:90,borderRadius:"50%",margin:"0 auto 20px",
          ...
          transform:`rotate(${tick%2===0?-14:14}deg)`}}>🔔</div>
        <div style={{fontSize:11,fontWeight:700,letterSpacing:4,color:"#7c3aed",
          textTransform:"uppercase",marginBottom:10}}>⏰ ALARM RINGING</div>
        <div style={{fontSize:28,fontWeight:900,color:dark?"#f1f0fb":"#1e1b4b",
          lineHeight:1.25,marginBottom:14,letterSpacing:"-0.5px"}}>{todo.text}</div>
        <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <span style={{fontSize:13,fontWeight:700,padding:"5px 14px",borderRadius:99,
            color:"#fff",background:cm?cm.gradient:"linear-gradient(135deg,#6366f1,#818cf8)"}}>
            {cm?cm.icon:"✨"} {todo.category}
          </span>
          {todo.alarm&&<span style={{fontSize:13,fontWeight:700,padding:"5px 14px",borderRadius:99,
            color:"#7c3aed",background:dark?"rgba(124,58,237,0.15)":"#ede9fe",border:"1px solid #ddd6fe"}}>
            🕐 {todo.alarm}
          </span>}
          <span style={{fontSize:13,fontWeight:700,padding:"5px 14px",borderRadius:99,
            color:todo.voiceType==="female"?"#ec4899":"#3b82f6",
            background:todo.voiceType==="female"?(dark?"rgba(236,72,153,0.15)":"#fdf2f8"):(dark?"rgba(59,130,246,0.15)":"#eff6ff"),
            border:`1px solid ${todo.voiceType==="female"?"#fbcfe8":"#bfdbfe"}`}}>
            {todo.voiceType==="female"?"👩 Female":"👨 Male"}
          </span>
        </div>
        <button onClick={()=>speakAlarm(todo.text, todo.voiceType||"male")} style={{
          width:"100%",padding:"13px",borderRadius:14,marginBottom:10,
          border:"2px solid #7c3aed",background:dark?"rgba(124,58,237,0.15)":"#faf5ff",
          color:"#7c3aed",fontSize:14,fontWeight:700,cursor:"pointer"}}>
          🔊 Speak Again
        </button>
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>onSnooze(todo.id)} style={{flex:1,padding:"13px",borderRadius:14,
            border:`1px solid ${dark?"rgba(255,255,255,0.1)":"#e0e7ff"}`,
            background:dark?"rgba(255,255,255,0.05)":"#f0f4ff",
            color:dark?"#a5b4fc":"#6366f1",fontSize:13,fontWeight:700,cursor:"pointer"}}>
            😴 Snooze 5 min
          </button>
          <button onClick={()=>onDismiss(todo.id)} style={{flex:1,padding:"13px",borderRadius:14,
...
// ─────────────────────────────────────────────────────────────────────────────
// EDIT MODAL — edit task text, category, priority, due date, alarm, voice
// ─────────────────────────────────────────────────────────────────────────────
function EditModal({ todo, onSave, onClose, dark }) {
  ...
  const T = {
    bg:   dark?"#13131f":"#fff", text: dark?"#f1f0fb":"#1e1b4b",
    muted:dark?"#9ca3af":"#6b7280", bord:dark?"rgba(139,92,246,0.2)":"rgba(99,102,241,0.2)",
    inp:  dark?"rgba(255,255,255,0.07)":"#f9f8ff", sub:"#7c3aed",
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:10001,background:"rgba(0,0,0,0.7)",
      ...
      onClick={onClose}>
      <div style={{background:T.bg,borderRadius:28,padding:28,maxWidth:460,width:"100%",
        border:`1.5px solid ${T.bord}`,boxShadow:"0 24px 80px rgba(0,0,0,0.5)",
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:"transparent",border:"none",
            fontSize:20,cursor:"pointer",color:T.muted,lineHeight:1}}>✕</button>
        </div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input value={text} onChange={e=>setText(e.target.value)}
            style={{width:"100%",padding:"11px 14px",borderRadius:12,border:`1.5px solid ${T.bord}`,
              background:T.inp,color:T.text,fontSize:15,fontWeight:600,outline:"none",
        ...
        <div style={{display:"flex",gap:10,marginBottom:14}}>
          <div style={{flex:1}}>
            ...
            <select value={category} onChange={e=>setCategory(e.target.value)} style={{
              width:"100%",padding:"10px 12px",borderRadius:12,border:`1.5px solid ${T.bord}`,
              background:T.inp,color:T.text,fontSize:13,fontWeight:600,outline:"none",fontFamily:"inherit"}}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:14}}>
          <div style={{flex:1}}>
            ...
            <input type="date" value={due} onChange={e=>setDue(e.target.value)} style={{
              width:"100%",padding:"10px 12px",borderRadius:12,border:`1.5px solid ${T.bord}`,
              background:T.inp,color:T.text,fontSize:13,fontWeight:600,outline:"none",
              fontFamily:"inherit",boxSizing:"border-box"}}/>
          </div>
          <div style={{flex:1}}>
            ...
            <input type="time" value={alarm} onChange={e=>setAlarm(e.target.value)} style={{
              width:"100%",padding:"10px 12px",borderRadius:12,border:`1.5px solid ${alarm?"#7c3aed":T.bord}`,
              background:T.inp,color:T.text,fontSize:14,fontWeight:700,outline:"none",
              fontFamily:"inherit",boxSizing:"border-box",letterSpacing:1}}/>
          </div>
        </div>
        ...
        <div style={{marginBottom:20}}>
          ...
          <div style={{display:"flex",gap:10}}>
            {[
              {val:"male",   icon:"👨", label:"Male Voice",   color:"#3b82f6", bg:dark?"rgba(59,130,246,0.15)":"#eff6ff", bord:"#bfdbfe"},
              {val:"female", icon:"👩", label:"Female Voice", color:"#ec4899", bg:dark?"rgba(236,72,153,0.15)":"#fdf2f8", bord:"#fbcfe8"},
            ].map(v=>(
              <button key={v.val} onClick={()=>setVoiceType(v.val)} style={{
                flex:1,padding:"12px 8px",borderRadius:14,cursor:"pointer",
                border:`2px solid ${voiceType===v.val?v.color:T.bord}`,
                background:voiceType===v.val?v.bg:(dark?"transparent":"#fafafa"),
                transition:"all 0.2s",
              }}>
                ...
                {voiceType===v.val&&(
                  <button onClick={e=>{e.stopPropagation();speakAlarm(text||"Test",v.val);}} style={{
                    marginTop:6,padding:"4px 10px",borderRadius:8,border:"none",
                    background:v.color,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",
                  }}>🔊 Test</button>
                )}
              </button>
            ))}
        ...
        {/* Save / Cancel */}
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:"13px",borderRadius:14,
            border:`1px solid ${T.bord}`,background:"transparent",
            color:T.muted,fontSize:14,fontWeight:700,cursor:"pointer"}}>
            Cancel
          </button>
          <button onClick={()=>onSave({text:text.trim()||todo.text,category,priority,due,alarm,voiceType})} style={{
...
export default function TodoApp() {
  ...
  const [todos,      setTodos]     = useState(()=>{try{return JSON.parse(localStorage.getItem("todo-v6"))||DEFAULTS;}catch{return DEFAULTS;}});
  ...
  useEffect(()=>{ localStorage.setItem("todo-v6",JSON.stringify(todos)); },[todos]);
  ...
  useEffect(()=>{
    if(window.speechSynthesis){
      ...
      window.speechSynthesis.getVoices().length ? load() :
        ...
        const updated=prev.map(t=>{
          ...
          if(t.alarm===now){
            ...
            if("Notification" in window && Notification.permission==="granted"){
              new Notification(`⏰ ${t.text}`,{body:t.text,icon:"/logo192.png"});
            }
  ...
  const snoozeAlarm=(id)=>{
    window.speechSynthesis?.cancel(); setRinging(r=>r.filter(x=>x.id!==id));
  ...
  const saveEdit=(id, changes)=>{
    setTodos(p=>p.map(t=>t.id===id?{...t,...changes,alarmFired:false}:t));
    setEditTodo(null);
  };
  ...
  const T={
    bg:dark?"#0b0b14":"#f4f3ff", card:dark?"rgba(255,255,255,0.045)":"rgba(255,255,255,0.9)",
    glass:dark?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.75)",
    text:dark?"#ede9fe":"#1e1b4b", sub:dark?"#a78bfa":"#7c3aed",
    muted:dark?"#6b7280":"#9ca3af", bord:dark?"rgba(139,92,246,0.15)":"rgba(99,102,241,0.15)",
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",
      color:T.text,transition:"background 0.4s",position:"relative",overflowX:"hidden"}}>

      <Confetti active={confetti} onDone={()=>setConfetti(false)}/>
      <AlarmPopup alarms={ringing} onDismiss={dismissAlarm} onSnooze={snoozeAlarm} dark={dark}/>
      {editTodo&&<EditModal todo={editTodo} dark={dark}
        onSave={(changes)=>saveEdit(editTodo.id,changes)}
        onClose={()=>setEditTodo(null)}/>}

      {/* BG Orbs */}
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
        {[{top:"-15%",left:"-10%",s:580,c:dark?"rgba(99,102,241,0.12)":"rgba(99,102,241,0.18)"},
          {bottom:"-15%",right:"-10%",s:500,c:dark?"rgba(236,72,153,0.08)":"rgba(236,72,153,0.12)"},
          {top:"40%",right:"15%",s:280,c:dark?"rgba(56,189,248,0.07)":"rgba(56,189,248,0.1)"}
        ].map((o,i)=>(
          <div key={i} style={{position:"absolute",...(o.top?{top:o.top}:{bottom:o.bottom}),
            ...(o.left?{left:o.left}:{right:o.right}),width:o.s,height:o.s,borderRadius:"50%",
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:640,margin:"0 auto",padding:"28px 16px 100px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}>
          <div>
            <div style={{fontSize:11,fontWeight:700,letterSpacing:4,textTransform:"uppercase",
              background:"linear-gradient(90deg,#6366f1,#ec4899)",
              WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:8}}>
              ✦ TASK MANAGER ✦
            </div>
            <h1 style={{fontSize:34,fontWeight:900,margin:0,letterSpacing:"-1.5px",lineHeight:1,
              background:dark?"linear-gradient(135deg,#c4b5fd,#f9a8d4)":"linear-gradient(135deg,#4338ca,#7c3aed)",
          ...
          <button onClick={()=>setDark(d=>!d)} style={{width:46,height:46,borderRadius:15,
            border:`1px solid ${T.bord}`,background:T.glass,backdropFilter:"blur(12px)",
            cursor:"pointer",fontSize:22,display:"flex",alignItems:"center",justifyContent:"center",
            boxShadow:T.shadow,flexShrink:0}}>{dark?"☀️":"🌙"}</button>
        </div>

        {/* PROGRESS */}
        <div style={{background:T.card,backdropFilter:"blur(20px)",borderRadius:28,
          border:`1px solid ${T.bord}`,padding:22,marginBottom:16,boxShadow:T.shadow}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
            {[{n:total,l:"Total",c:"#6366f1",bg:dark?"rgba(99,102,241,0.2)":"#eef2ff"},
              {n:doneCnt,l:"Done",c:"#10b981",bg:dark?"rgba(16,185,129,0.2)":"#ecfdf5"},
              {n:total-doneCnt,l:"Left",c:"#f59e0b",bg:dark?"rgba(245,158,11,0.2)":"#fefce8"},
              {n:overdueCnt,l:"Overdue",c:"#f43f5e",bg:dark?"rgba(244,63,94,0.2)":"#fff1f2"},
              {n:alarmCnt,l:"Alarms",c:"#a855f7",bg:dark?"rgba(168,85,247,0.2)":"#faf5ff"},
            ].map(({n,l,c,bg:sb})=>(
              <div key={l} style={{background:sb,borderRadius:16,padding:"12px 6px",
        ...
        {/* SEARCH */}
        <div style={{display:"flex",alignItems:"center",gap:10,background:T.glass,
          backdropFilter:"blur(20px)",border:`1px solid ${T.bord}`,borderRadius:18,
          padding:"11px 16px",marginBottom:12,boxShadow:T.shadow}}>
          <span style={{fontSize:16,flexShrink:0}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search tasks..."
            style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:14,color:T.text,fontFamily:"inherit"}}/>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
          {CATEGORIES.map(c2=>{
            ...
            return(<button key={c2} onClick={()=>setFilter(c2)} style={{
              padding:"9px 18px",borderRadius:14,border:"none",cursor:"pointer",
              fontSize:13,fontWeight:700,transition:"all 0.2s",
              background:a?(m?m.gradient:"linear-gradient(135deg,#6366f1,#818cf8)"):T.glass,
              color:a?"#fff":T.muted,backdropFilter:"blur(10px)",
              boxShadow:a?`0 4px 16px ${m?m.glow:"rgba(99,102,241,0.3)"}`:"none",
              transform:a?"translateY(-2px)":"none",
            }}>{m?`${m.icon} ${c2}`:`✨ ${c2}`}</button>);
          })}
          <div style={{display:"flex",gap:6,marginLeft:"auto"}}>
            {PRIORITIES.map(p2=>{
              const a=priFilter===p2; const m=PRI_META[p2];
              return(<button key={p2} onClick={()=>setPriFilter(priFilter===p2?"All":p2)} style={{
                padding:"7px 12px",borderRadius:12,cursor:"pointer",fontSize:12,fontWeight:700,
                border:`1.5px solid ${a?m.color:T.bord}`,
                background:a?m.bg:T.glass,color:a?m.color:T.muted,backdropFilter:"blur(10px)",
              }}>{m.label}</button>);
            })}
          </div>
        </div>
        ...
        {/* ADD TASK */}
        <div style={{background:T.card,backdropFilter:"blur(20px)",marginBottom:14,
          border:`1.5px solid ${showAdd?"#7c3aed":T.bord}`,borderRadius:24,overflow:"hidden",
          boxShadow:showAdd?"0 12px 40px rgba(124,58,237,0.2)":T.shadow,transition:"all 0.3s"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 18px"}}>
            <div onClick={()=>setShowAdd(s=>!s)} style={{width:36,height:36,borderRadius:12,
              flexShrink:0,cursor:"pointer",background:"linear-gradient(135deg,#6366f1,#ec4899)",
              display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:22,color:"#fff",fontWeight:700,lineHeight:1,
              boxShadow:"0 4px 14px rgba(99,102,241,0.4)"}}>
              {showAdd?"−":"+"}
            </div>
            <input value={input} onChange={e=>setInput(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&addTodo()} onFocus={()=>setShowAdd(true)}
              placeholder="What do you need to do?"
              style={{flex:1,background:"transparent",border:"none",outline:"none",
                fontSize:16,color:T.text,fontFamily:"inherit",fontWeight:500}}/>
            {input&&<button onClick={addTodo} style={{
              background:"linear-gradient(135deg,#6366f1,#818cf8)",border:"none",
              borderRadius:12,padding:"9px 18px",color:"#fff",fontSize:13,fontWeight:800,
              cursor:"pointer",whiteSpace:"nowrap",boxShadow:"0 4px 14px rgba(99,102,241,0.4)"}}>
              Add ↵
            </button>}
          </div>
          ...
          {showAdd&&(
            <div style={{padding:"4px 18px 18px",borderTop:`1px solid ${T.bord}`}}>
                ...
                {[
                  ...
                  <div key={i} style={{flex:1,minWidth:120}}>
                    ...
                    <select value={s.val} onChange={e=>s.set(e.target.value)} style={{
                      width:"100%",padding:"9px 12px",borderRadius:12,border:`1.5px solid ${T.bord}`,
                      background:T.inp,color:T.text,fontSize:13,fontWeight:600,outline:"none",fontFamily:"inherit"}}>
              ...
              {/* Voice Alarm */}
              <div style={{background:dark?"rgba(124,58,237,0.1)":"#faf5ff",
                border:`1.5px solid ${dark?"rgba(124,58,237,0.25)":"#ddd6fe"}`,borderRadius:18,padding:16}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  <div style={{width:38,height:38,borderRadius:12,flexShrink:0,
                    background:"linear-gradient(135deg,#7c3aed,#a855f7)",
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,
                    boxShadow:"0 4px 12px rgba(124,58,237,0.3)"}}>🔔</div>
                  <div>
                    <div style={{fontSize:14,fontWeight:800,color:"#7c3aed"}}>Voice Alarm</div>
                    <div style={{fontSize:11,color:T.muted}}>Speaks your task name at set time</div>
                  </div>
                </div>

                {/* Time picker */}
                <input type="time" value={alarm} onChange={e=>setAlarm(e.target.value)} style={{
                  width:"100%",padding:"12px 16px",borderRadius:14,boxSizing:"border-box",marginBottom:12,
                  border:`2px solid ${alarm?"#7c3aed":T.bord}`,
                  background:T.inp,color:T.text,fontSize:18,fontWeight:800,
                  outline:"none",fontFamily:"inherit",letterSpacing:2}}/>

                {/* Voice picker */}
                <div style={{display:"flex",gap:8}}>
                  {[
                    {val:"male",  icon:"👨",label:"Male",   color:"#3b82f6",bg:dark?"rgba(59,130,246,0.15)":"#eff6ff",bord:"#bfdbfe"},
                    {val:"female",icon:"👩",label:"Female", color:"#ec4899",bg:dark?"rgba(236,72,153,0.15)":"#fdf2f8",bord:"#fbcfe8"},
                  ].map(v=>(
                    <button key={v.val} onClick={()=>setVoiceType(v.val)} style={{
                ...
                {alarm&&(
                  <div style={{marginTop:10,padding:"10px 14px",borderRadius:12,
                    ...
                    🔊 At <strong>{alarm}</strong> — will speak: <em>"{input||"your task"}"</em>
                  </div>
                )}
                {input&&(
                  <button onClick={()=>speakAlarm(input,voiceType)} style={{
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {visible.length===0&&(
            <div style={{textAlign:"center",padding:"56px 24px",background:T.glass,
              backdropFilter:"blur(20px)",borderRadius:24,border:`1px solid ${T.bord}`}}>
              <div style={{fontSize:52,marginBottom:14}}>✨</div>
              <div style={{fontSize:18,fontWeight:700,color:T.text,marginBottom:8}}>
                {search?`No tasks matching "${search}"`:"You're all caught up!"}
              </div>
          ...
          {visible.map(todo=>{
            ...
            return(
              <div key={todo.id} draggable
                ...
                style={{background:todo.done?(dark?"rgba(255,255,255,0.02)":"rgba(255,255,255,0.6)"):T.card,
                  backdropFilter:"blur(20px)",borderRadius:20,
                  border:`1.5px solid ${isDragTarget?"#6366f1":hasAlarm?"rgba(124,58,237,0.4)":T.bord}`,
                  padding:"15px 16px",display:"flex",alignItems:"flex-start",gap:12,
                  opacity:dragId===todo.id?0.3:1,
                  transform:isDragTarget?"scale(1.02)":"scale(1)",
                  transition:"all 0.2s ease",
                  boxShadow:hasAlarm?(dark?"0 6px 28px rgba(124,58,237,0.22)":"0 6px 28px rgba(124,58,237,0.14)"):
                    todo.done?"none":T.shadow,
                  cursor:"grab",position:"relative",overflow:"hidden"}}>

                {!todo.done&&<div style={{position:"absolute",left:0,top:0,bottom:0,width:4,
                  background:hasAlarm?"linear-gradient(180deg,#7c3aed,#a855f7)":(cm?cm.gradient:"linear-gradient(#6366f1,#ec4899)"),
                  borderRadius:"20px 0 0 20px"}}/>}

                <span style={{color:T.muted,fontSize:15,marginTop:2,cursor:"grab",userSelect:"none",flexShrink:0,paddingLeft:6}}>⠿</span>

                <div onClick={()=>toggle(todo.id)} style={{width:24,height:24,borderRadius:9,flexShrink:0,
                  border:todo.done?"none":`2px solid ${dark?"rgba(255,255,255,0.2)":"#c4b5fd"}`,
                  background:todo.done?"linear-gradient(135deg,#6366f1,#ec4899)":"transparent",
                  cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
                  transition:"all 0.2s",boxShadow:todo.done?"0 3px 10px rgba(99,102,241,0.45)":"none",marginTop:0}}>
                  {todo.done&&<span style={{color:"#fff",fontSize:13,fontWeight:700}}>✓</span>}
                </div>

                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:15,fontWeight:600,lineHeight:1.4,wordBreak:"break-word",
                    color:todo.done?T.muted:T.text,textDecoration:todo.done?"line-through":"none",
                    opacity:todo.done?0.6:1}}>{todo.text}</div>
                  <div style={{display:"flex",gap:6,marginTop:9,flexWrap:"wrap",alignItems:"center"}}>
                    ...
                    {todo.due&&<span style={{fontSize:11,padding:"4px 10px",borderRadius:99,fontWeight:600,
                      background:isOverdue?"#fff1f2":(dark?"rgba(255,255,255,0.07)":"#f5f3ff"),
                      color:isOverdue?"#f43f5e":T.muted,
                      border:`1px solid ${isOverdue?"#fecdd3":T.bord}`}}>
                      📅 {todo.due}
                    </span>}
                    {todo.alarm&&!todo.done&&<span style={{fontSize:11,padding:"4px 10px",borderRadius:99,
                      fontWeight:700,background:dark?"rgba(124,58,237,0.18)":"#faf5ff",
                      color:"#7c3aed",border:"1.5px solid #ddd6fe",
                      display:"flex",alignItems:"center",gap:5}}>
                      {isFemale?"👩":"👨"} 🔔 {todo.alarm}
                      <span onClick={e=>{e.stopPropagation();speakAlarm(todo.text,todo.voiceType||"male");}}
                        style={{background:"#7c3aed",color:"#fff",borderRadius:6,
                          padding:"1px 6px",fontSize:10,cursor:"pointer",fontWeight:700}}>🔊</span>
                    </span>}
                  </div>
                </div>
                ...
                <div style={{display:"flex",gap:5,flexShrink:0}}>
                  <button onClick={()=>setEditTodo(todo)} style={{width:34,height:34,borderRadius:11,
                    border:`1px solid ${T.bord}`,background:T.glass,cursor:"pointer",fontSize:14,
                    display:"flex",alignItems:"center",justifyContent:"center",color:T.muted,
                    backdropFilter:"blur(10px)"}}>✏️</button>
                  <button onClick={()=>remove(todo.id)} style={{width:34,height:34,borderRadius:11,
                    border:"1.5px solid #fecdd3",background:dark?"rgba(244,63,94,0.1)":"#fff1f2",
                    cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",
                    justifyContent:"center",color:"#f43f5e"}}>🗑️</button>
                </div>
              </div>
            );
          })}
        ...
        {todos.some(t=>t.done)&&(
          <button onClick={()=>setTodos(p=>p.filter(t=>!t.done))} style={{
            width:"100%",marginTop:14,padding:"13px",background:T.glass,backdropFilter:"blur(10px)",
            border:`1.5px dashed ${T.bord}`,borderRadius:16,color:T.muted,fontSize:13,fontWeight:700,cursor:"pointer"}}>
